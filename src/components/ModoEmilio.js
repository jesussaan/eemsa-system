      ...
      <main style={{ flex: 1, padding: "16px 16px 32px", maxWidth: 640, margin: "0 auto", width: "100%" }}>
        ...
        {gruposPendientes.map(grupo => grupo.length > 1 ? (
          ...
          <div key={grupo[0].id} style={{ ...S.card, borderLeft: `4px solid ${REBOB_COLOR}` }}>
            ...
            {/* ── INSUMOS (solo "Centros utilizados" se desglosa por ancho --
                 son piezas de tamaño distinto si cambia entre 2" y 3", asi
                 que sumarlas de corrido no dice nada util; lo demas se sigue
                 juntando igual que siempre) ── */}
            {(() => {
              const anchoDe = (p) => String(p.medida || '').split(' x ')[0];
              const anchosUnicos = [...new Set(grupo.map(anchoDe).filter(Boolean))];
              const centrosPorAncho = anchosUnicos.length > 1 ? anchosUnicos.map(a => ({
                ancho: a,
                centros: grupo.filter(p => anchoDe(p) === a).reduce((s, p) => s + (Number(p.piezas_prod) || 0) + (Number(p.merma) || 0), 0),
              })) : null;
              return (
                <div style={S.section}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    <div style={S.mini}>
                      <div style={S.miniLbl}>Centros utilizados{centrosPorAncho ? " (total)" : ""}</div>
          ...
          const cajasProducidas = esRebobinado
            ...
            : (p.piezas_prod != null && p.rollos_caja
              ...
              : null);
          // Piezas sueltas (las que no alcanzan a llenar una caja) ya NO
          // cuentan como una caja de carton consumida -- "Cajas utilizadas"
          // es siempre igual a "Cajas producidas" (cajas completas nada mas).
          const cajasUtilizadas = cajasProducidas;
