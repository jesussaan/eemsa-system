        ...
        {pendientes.map(p => (
          <div key={p.id} style={card}>
            ...
            <div style={{ background: "#0d0f14", borderRadius: 10, padding: 12, marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: "#c9922a", fontWeight: 700, letterSpacing: ".06em", marginBottom: 8, textTransform: "uppercase" }}>🧮 Calculado al finalizar</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                ...
                {p.merma_pct != null && p.merma_pct !== "" && <div style={miniCard}><div style={miniLbl}>% Merma</div><div style={{ color: Number(p.merma_pct) > 3 ? "#ff4d4d" : "#4be87a", fontWeight: 700, fontSize: 16 }}>{Number(p.merma_pct).toFixed(2)}%</div></div>}
                {p.rollos_usados != null && p.rollos_usados !== "" && <div style={miniCard}><div style={miniLbl}>Rollos de MP</div><div style={{ color: "#4b8fe8", fontWeight: 700, fontSize: 18 }}>{p.rollos_usados}</div></div>}
                {p.tinta_kg != null && p.tinta_kg !== "" && <div style={miniCard}><div style={miniLbl}>Tinta estimada</div><div style={{ color: "#c9922a", fontWeight: 700, fontSize: 18 }}>{Number(p.tinta_kg).toFixed(3)} kg</div></div>}
                {p.alcohol_litros != null && p.alcohol_litros !== "" && <div style={miniCard}><div style={miniLbl}>Solvente estimado</div><div style={{ color: "#4b8fe8", fontWeight: 700, fontSize: 18 }}>{Number(p.alcohol_litros).toFixed(3)} kg</div></div>}
